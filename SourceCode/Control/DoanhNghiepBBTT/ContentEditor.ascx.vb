...
#Region "Sub and Function "
    ...
    Protected Sub setPermission()
        lbtView.Visible = HasPermission(Function_Name.DoanhNghiepBBTT, Session("RoleID"), 0, Audit_Type.ViewContent)
        lbtEdit.Visible = HasPermission(Function_Name.DoanhNghiepBBTT, Session("RoleID"), 0, Audit_Type.Edit)
        btnDelete.Visible = HasPermission(Function_Name.DoanhNghiepBBTT, Session("RoleID"), 0, Audit_Type.Delete)
    End Sub
    ...
    Protected Sub lbtBBTT_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtBBTT.Click
        TaoPhieuHeader(1)
        Response.Redirect("../BienBanThanhTra/ThongTinChung.aspx?DNId=" & hidID.Value)
    End Sub
                ...
                If isUser = UserType.Admin Or UserName = dn.NguoiTao Then '' Có quyền Xóa
                    ...
                    Try
                        ...
                        If pn.Count = 0 Then
                            ...
                            data.SaveChanges()
                            Insert_App_Log("Delete Doanh Nghiep:" & intId & "", Function_Name.DoanhNghiepBBTT, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
                            Excute_Javascript("Alertbox('Xóa dữ liệu thành công.');window.location ='../../Page/DoanhNghiepBBTT/List.aspx';", Me.Page, True)
                        Else
    ...
    Protected Sub TaoPhieuHeader(ByVal _Type As Integer)
        ...
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim qdtt = (From a In data.QuyetDinhThanhTras
                      Join b In data.QuyetDinhTTDoanhNghieps On a.SoQuyetDinh Equals b.QuyetDinhTT
                Where b.DoanhNghiepId = hidID.Value Select a).FirstOrDefault
            If Not IsNothing(p) Then
                ...
                If Not IsNothing(qdtt) Then
                    pn.SoQuyenDinh = qdtt.SoQuyetDinh
                End If
                'Chuyển Thành viên Đoàn của quyết định thanh tra sang Thành phần tham gia của biên bản thanh tra
                pn.ThanhPhanThamGia = qdtt.ThanhVienDoan
                pn.NgayTao = Date.Now
