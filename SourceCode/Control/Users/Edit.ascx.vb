    ...
    Private Sub LoadTinh()
        Using data As New ThanhTraLaoDongEntities
            ...
            If hidTinhThanhTraSo.Value = 0 Then
                p = (From a In data.Tinhs Select a.TinhId, a.TenTinh).ToList()
                With ddlTinh
                    .DataValueField = "TinhId"
                    .DataTextField = "TenTinh"
                    .DataSource = p
                    .DataBind()
                    Dim lstItem As New ListItem("--- Chọn ---", "")
                    ddlTinh.Items.Insert(0, lstItem)
                End With
            Else
                p = (From a In data.Tinhs Where a.TinhId = hidTinhThanhTraSo.Value Select a.TinhId, a.TenTinh).ToList()
                With ddlTinh
                    .DataValueField = "TinhId"
                    .DataTextField = "TenTinh"
                    .DataSource = p
    ...
    Protected Sub ShowData()
        Using data As New ThanhTraLaoDongEntities
            ...
            If Not p Is Nothing Then
                ...
                ddlTinh.SelectedValue = IIf(IsNothing(p.TinhTP), 0, p.TinhTP)
