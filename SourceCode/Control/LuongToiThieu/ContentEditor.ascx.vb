...
Partial Class Control_LuongToiThieu_ContentEditor
...
#Region "Sub and Function "
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            If Request.PhysicalPath.Contains("List.aspx") Then
                ...
                'Chi ap dung cho du lieu xuat banm
                'tabs_state.Visible = False
                'cboAction.Visible = False

                'Permission
                lbtAdd.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.Create)
            ...
            ElseIf Request.PhysicalPath.Contains("Edit.aspx") Then
                ...
                setPermission()
            End If
            If Not Request.QueryString("LTTID") Is Nothing Then
                hidID.Value = Request.QueryString("LTTID")
                'Load list action status
                LoadListStatus(hidID.Value)
                'Permission
                btnDelete.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.Delete)
    ...
    Protected Sub setPermission()
        ...
        lbtAdd.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.Create)
        lbtView.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.ViewContent)
        lbtEdit.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.Edit)
        btnDelete.Visible = HasPermission(Function_Name.LuongToiThieu, Session("RoleID"), 0, Audit_Type.Delete)
    End Sub
