...
Partial Class Control_Quyetdinhthanhtra_ContentEditor
...
#Region "Sub and Function "
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            If Request.PhysicalPath.Contains("List.aspx") Then
                ...
                'Permission
                'lbtAdd.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.Create)
            ...
            ElseIf Request.PhysicalPath.Contains("Edit.aspx") Then
                ...
                'setPermission()
            End If
            If Not Request.QueryString("SoQuyetDinh") Is Nothing Then
                hidID.Value = Request.QueryString("SoQuyetDinh")
                'Load list action status
                LoadListStatus(hidID.Value)
                'Permission
                'btnDelete.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.Delete)
    ...
    Protected Sub setPermission()
        ...
        'lbtAdd.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.Create)
        'lbtView.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.ViewContent)
        'lbtEdit.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.Edit)
        'btnDelete.Visible = HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), 0, Audit_Type.Delete)
    End Sub
    ...
    Protected Sub lbtEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtEdit.Click
        Response.Redirect("Edit.aspx?SoQuyetDinh=" & hidID.Value & "")
    End Sub
    Protected Sub lbtView_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtView.Click
        Response.Redirect("Detail.aspx?SoQuyetDinh=" & hidID.Value & "")
    End Sub
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        ...
        Using data As New thanhtralaodongEntities
            intId = hidID.Value
            Dim q = (From p In Data.Quyetdinhthanhtras Where p.Id = intId Select p).FirstOrDefault
            Try
                ...
                data.SaveChanges()
                Excute_Javascript("AlertboxRedirect('Xóa dữ liệu thành công.','../../Page/QuyetdinhthanhtraTTS/List.aspx');", Me.Page, True)
    ...
    Sub LoadListStatus(ByVal iQuyetdinhthanhtraId As String)
        'Using Data As New thanhtralaodongEntities
        'Get Current StatusId of object
        'Dim cst = (From q In Data.Quyetdinhthanhtras Where q.Id = iQuyetdinhthanhtraId Select q).SingleOrDefault
        ...
        'For Each i In st
        'If HasPermission(Function_Name.Quyetdinhthanhtra, Session("RoleID"), i.CurrentStatusId, i.Audit) Then
