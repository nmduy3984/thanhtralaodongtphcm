<%@ Control Language="VB" AutoEventWireup="false" CodeFile="KetQuaThanhTra1.ascx.vb"
    Inherits="Control_Report_KetQuaThanhTra1" %>
<script type="text/javascript">
    ...
    function SetHeader() {
        ...
        reportName.text("BÁO CÁO TỔNG HỢP KẾT QUẢ THANH TRA " + ($("#<%=ddlProvince.ClientID%>").val() * 1 == -1 ? "TOÀN QUỐC" : ("TỈNH  " + tinh))
                                                            + (tuNam == "" ? "" : (" TỪ NĂM  " + tuNam))
                                                            + (denNam == "" ? "" : ("   ĐẾN NĂM  " + denNam)));
    }
    //kiểm tra user nhập
