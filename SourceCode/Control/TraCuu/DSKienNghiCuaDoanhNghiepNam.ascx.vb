...
Partial Class Control_TraCuu_DSKienNghiCuaDoanhNghiepNam
...
#Region "PRIVATE EVENT FOR CONTROL"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1 _
        ...
        Using data As New ThanhTraLaoDongEntities
            Dim arrSearch() As String = {iPage.ToString, strTinh.ToString, strYearFrom.ToString}

            ViewState("search") = arrSearch
            'So ban ghi muon the hien tren trang
            Dim intPag_Size As Int32 = drpPage_Size.SelectedValue
            Dim p = data.uspTraCuuDSKienNghiCuaDoanhNghiepNam(strTinh, strYearFrom, iPage, intPag_Size).ToList()
            'Tong so ban ghi
            Dim strKey_Name() As String = {"DoanhNghiepId", "NDKienNghi"}
            'Tong so ban ghi
            If p.Count > 1 Then
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            Dim lblNoiDung As Label = CType(e.Row.FindControl("lblNoiDung"), Label)
            lblNoiDung.Text = grdShow.DataKeys(e.Row.RowIndex)("NDKienNghi").ToString.Replace("#", "</br>")
    ...
    Protected Sub lnkTitle_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim arrSearch() As String
        arrSearch = ViewState("search")
        BindToGrid(CInt(lnkTile.ToolTip.ToString), arrSearch(1), arrSearch(2))
