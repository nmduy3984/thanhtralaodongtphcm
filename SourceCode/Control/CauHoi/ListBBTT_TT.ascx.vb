...
Partial Class Control_CauHoi_ListBBTT_TT
...
#Region "Sub and Function"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1,
        ...
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim p = data.uspPhieuNhapHeaderSelectAll(strSearch, 1, hidUserName.Value, hidIsUser.Value, hidUserId.Value, strNguoiTao, strTrangThai, strNgayTaoTu, strNgayTaoDen, iPage, intPag_Size).ToList
            ...
            End If
            'Thống kê nhanh
            Dim thongke = data.uspThongKeBBTT(Session("UserName"), CInt(Session("UserId"))).SingleOrDefault
            If Not IsNothing(thongke) Then
                lblThongKe.Text = "<ul><b>Thống kê nhanh</b>"
                lblThongKe.Text += "<li>1. Số DN đang được thanh tra: <span style='color:blue;'><b>" & String.Format(info, "{0:n0}", thongke.CountBBTTChuaHT) & "</b></span><li>"
                lblThongKe.Text += "<li>2. Số DN đã được thanh tra: <span style='color:blue;'><b>" & String.Format(info, "{0:n0}", thongke.CountBBTTDaHT) & "</b></span><li>"
                lblThongKe.Text += "<li>3. Tổng số DN bị xử lý vi phạm: <span style='color:blue;'><b>" & String.Format(info, "{0:n0}", thongke.CountDNBiXLVP) & "</b></span><li>"
                lblThongKe.Text += "<li>4. Tổng số tiền DN bị xử lý vi phạm: <span style='color:blue;'><b>" & String.Format(info, "{0:n0}", thongke.SumMucPhatDN) & "</b></span> đồng<li>"
                lblThongKe.Text += "</ul>"
            End If
...
#Region "Event for control"
    Protected Function DeletePhieu(ByVal iPhieuId As Integer) As Boolean
        ...
        Using data As New ThanhTraLaoDongEntities
            ...
            Try
                ...
                data.SaveChanges()
                Insert_App_Log("Delete  Phieunhapheader:" & strTenPhieu & "", Function_Name.BienBanThanhTra_TT, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
                Return True
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Dim bResult As Boolean = DeletePhieu(grdShow.DataKeys(hidID.Value)("PhieuID"))
        If bResult Then
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            ...
            'Link KLTT
            Dim hplKLTT As HyperLink = CType(e.Row.FindControl("hplKLTT"), HyperLink)
            'Link KLTT
            Dim hplBBVP As HyperLink = CType(e.Row.FindControl("hplBBVP"), HyperLink)

            'Cau hoi da tra loi
            If grdShow.DataKeys(e.Row.RowIndex)("IsHoanThanh").ToString().Contains("Hoàn thành") _
                AndAlso (grdShow.DataKeys(e.Row.RowIndex)("NguoiTao").ToString().Contains(Session("Username")) _
                         Or Session("Username").Equals("admin")) Then
                hplBBTT.NavigateUrl = "../../Page/BienBanThanhTra/BienBanThanhTra.aspx?phieuId=" & grdShow.DataKeys(e.Row.RowIndex)("PhieuID")
                hplKLTT.NavigateUrl = "../../Page/BienBanThanhTra/PhieuKetLuan.aspx?phieuId=" & grdShow.DataKeys(e.Row.RowIndex)("PhieuID")
                hplBBVP.NavigateUrl = "../../Page/BienBanThanhTra/BienBanViPham.aspx?phieuId=" & grdShow.DataKeys(e.Row.RowIndex)("PhieuID")
            Else
                hplBBTT.Enabled = False
                hplKLTT.Enabled = False
                hplBBVP.Enabled = False
            End If

            'Dim strNguoiTao As String = grdShow.DataKeys(e.Row.RowIndex)("NguoiTao").ToString()
            'Dim iVaiTro As Boolean = grdShow.DataKeys(e.Row.RowIndex)("VaiTro") = 1
            ...
            'Permission
            Dim permissiondel As Boolean = HasPermission(Function_Name.BienBanThanhTra_TT, Session("RoleID"), 0, Audit_Type.Delete)
            ...
            End If
            Dim permissionedit As Boolean = HasPermission(Function_Name.BienBanThanhTra_TT, Session("RoleID"), 0, Audit_Type.Edit)
            If (Not policy Is Nothing AndAlso policy = "1") Or
            ...
            'Permission
            lnkTenphieu.Enabled = HasPermission(Function_Name.BienBanThanhTra_TT, Session("RoleID"), 0, Audit_Type.ViewContent)
            'Link tenphieu
            ...
            'Xử lý ẩn link khi user hiện tại đăng nhập không phải là user tạo biên bản
    ...
    Protected Sub TaoPhieuHeader(ByVal iPhieuIdOld As Integer)
        ...
        Using data As New ThanhTraLaoDongEntities
            ...
            If Not IsNothing(p) Then
                ...
                Session("ModePhieu") = ModePhieu.Edit
                Insert_App_Log("Insert  Phieunhapheader:" & pn.TenPhieu & "", Function_Name.BienBanThanhTra_TT, Audit_Type.Create, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
