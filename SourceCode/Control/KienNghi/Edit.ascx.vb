...
Imports Cls_Common
Partial Class Control_KienNghi_Editor
...
#Region "Sub and Function "
    ...
    Protected Sub ShowData()
        Using data As New ThanhTraLaoDongEntities
            Dim p As DanhMucKienNghi = (From q In data.DanhMucKienNghis Where q.KienNghiID = hidID.Value Select q).SingleOrDefault
            If Not p Is Nothing Then
                txtMota.Text = IIf(IsNothing(p.NoiDungKN) = True, "", p.NoiDungKN)
                ddlLoaihanhvi.SelectedValue = IIf(IsNothing(p.RootCauHoi) = True, "", p.RootCauHoi)
            End If
            'Hien thi cay muc vi pham
            dtListDMKNSelect = (From h In data.CauHoiKienNghis Where h.DanhMucKienNghiId = hidID.Value Select h).ToList
            FillParentMenu()
        End Using
    End Sub
#End Region
#Region "Event for control"
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Using data As New ThanhTraLaoDongEntities
            Dim p As DanhMucKienNghi = (From q In data.DanhMucKienNghis Where q.KienNghiID = hidID.Value).SingleOrDefault
            Try
                ...
                data.ExecuteStoreCommand("delete from CauHoiKienNghi where DanhMucKienNghiId={0}", New Object() {knID})
                GetParentNode()
