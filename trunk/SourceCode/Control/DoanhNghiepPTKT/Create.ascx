<%@ Control Language="VB" AutoEventWireup="false" CodeFile="Create.ascx.vb" Inherits="Control_DoanhNghiep_Create" %>
...
<script type="text/javascript">
    function ajaxJquery() {
        $(function () {
            ...
            //CheckSoDKKD();
            //Kiem tra trung, Load dữ liệu trùng
            //checkduplicate();
            ...
            CheckOnlyAndEffect();
            //Focus cho SoChungNhanDKKD
            $("#<%= txtSochungnhandkkd.ClientID %>").live("focusout", function () {
                var strSoDKKD = $(this).val();
                var strTenDoanhNghiep = $("#<%=txtTendoanhnghiep.ClientId %>").val();
                var strDiaChi = $("#<%=txtTrusochinh.ClientId %>").val();
                var iTinhId = $("#<%=ddlTinh.ClientId %>").val();
                var hidID = $("#<%=hidID.ClientId %>").val();
                $.ajax({
                    type: "POST", //Phuong thuc truyen du lieu luon la POST
                    url: '<%=ResolveUrl("~/Services/wsAutoComplete.asmx/KiemTraTrungDoanhNghiep")%>',
                    data: "{'strSoDKKD': '" + strSoDKKD + "', 'DoanhNghiepId': '" + hidID + "'}", //"{'strTenDoanhNghiep': '" + strTenDoanhNghiep + "', 'strDiaChi': '" + strDiaChi + "', 'iTinhId': '" + iTinhId + "', 'strSoDKKD': '" + strSoDKKD + "', 'DoanhNghiepId': '" + hidID + "'}", //thong so truyen vao
    ...
    function checkduplicate() {
        ...
        $("#<%= txtSochungnhandkkd.ClientID %>").live("focusout", function () {
            ...
            $.ajax({
                type: "POST", //Phuong thuc truyen du lieu luon la POST
                url: '<%=ResolveUrl("~/Services/wsAutoComplete.asmx/KiemTraTrungDoanhNghiep")%>',
                data: "{'strTenDoanhNghiep': '" + strTenDoanhNghiep + "', 'strDiaChi': '" + strDiaChi + "', 'iTinhId': '" + iTinhId + "', 'strSoDKKD': '" + strSoDKKD + "', 'DoanhNghiepId': '" + hidID + "'}", //thong so truyen vao
                dataType: "", //kieu du lieu tra ve
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    var resultReturn = eval("(" + result.d + ")");
        ...
        //focusout TenDoanhNghiep
        $("#<%= txtTendoanhnghiep.ClientID %>").live("focusout", function () {
            var strSoDKKD = $("#<%=txtSochungnhandkkd.ClientId %>").val();
            var strTenDoanhNghiep = $(this).val();
            var strDiaChi = $("#<%=txtTrusochinh.ClientId %>").val();
            var iTinhId = $("#<%=ddlTinh.ClientId %>").val();
            var hidID = $("#<%=hidID.ClientId %>").val();
            $.ajax({
        ...
        //focusout TruSoChinh
        $("#<%= txtTrusochinh.ClientID %>").live("focusout", function () {
            var strSoDKKD = $("#<%=txtSochungnhandkkd.ClientId %>").val();
            var strTenDoanhNghiep = $("#<%= txtTendoanhnghiep.ClientID %>").val();
            var strDiaChi = $(this).val();
            var iTinhId = $("#<%=ddlTinh.ClientId %>").val();
            var hidID = $("#<%=hidID.ClientId %>").val();
            $.ajax({
                type: "POST", //Phuong thuc truyen du lieu luon la POST
    ...
    <ContentTemplate>
        <div class="BoxField" id="edit">
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtCodeNamtldn" runat="server" CssClass='TextBox ToolTip'
                    DataType="requiredAndInteger" AssociatedLableText="Vui lòng nhập năm khai trương, không theo năm đổi tên doanh nghiệp"
                    MaxLength="4" MinimumValue="" Range="False" />
            </div>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtTrusochinh" runat="server" CssClass='TextBox ToolTip'
                    DataType="required" AssociatedLableText="số nhà, Đường, Phố, Thôn, Bản (không nhập Huyện, Tỉnh)" />
            </div>
            <div class="DivRow">
            </div>
            <div class="Div-Left">
                ...
                <cc1:ValidationDropdownlist ID="ddlTinh" runat="server" CssClass='DropDownList' Style="width: 248px"
                    AssociatedLableText="tỉnh hoặc nhập tên tỉnh" DataType="required" AutoPostBack="true" />
            </div>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtKhucongnghiep" runat="server" CssClass='TextBox' AssociatedLableText="khu công nghiệp" />
            </div>
            <div class="DivRow">
            </div>
            <%--<div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
                    <asp:Label ID="lblLoaihinhsxid" CssClass="TextLabel" runat="server" Text="4. Lĩnh vực sản xuất kinh doanh chủ yếu:" /><span
                        class="fieldRequired">&nbsp;</span>
                </div>
                <cc2:ValidationTextBox ID="txtLoaihinhSX" AssociatedLableText="chọn lĩnh vực cấp 3"
                    DataType="required" runat="server" CssClass='TextBox' Style="width: 200px;" />
                <asp:HyperLink ID="hpLHSX" runat="server" Text="Chọn" />
            </div>--%>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtTongsonhanvien" runat="server" CssClass='TextBox ToolTip'
                    AssociatedLableText="Tổng số người đang làm việc đến thời điểm kiểm tra" DataType="requiredAndInteger" />
            </div>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
...
" />
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtSolaodongnu" runat="server" CssClass='TextBox ToolTip'
                    AssociatedLableText="Nếu không có nữ nhập số 0" DataType="requiredAndInteger" />
            </div>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
            ...
            <div class="Div-Left">
                ...
                <cc2:ValidationTextBox ID="txtTonggiatrisp" runat="server" CssClass='TextBox' Style="width: 185px;"
                    AssociatedLableText="tổng giá trị sản phẩm" DataType="numeric" />(tỷ đồng)
            </div>
            <div class="Div-Left">
                <div class="DivLabelDoanhNghiep">
        ...
        <div style="display: none">
            ...
            <asp:HiddenField ID="hidResult" Value="0" runat="server" />
        </div>
    </ContentTemplate>
</asp:UpdatePanel>
