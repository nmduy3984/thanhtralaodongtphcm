...
Partial Class Control_Function_List
...
#Region "Sub and Function"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1, Optional ByVal strSearch As String = "")
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim p = data.uspFunctionSelectAll(strSearch, iPage, intPag_Size).ToList

            Dim strKey_Name() As String = {"FunctionId", "FunctionName"}
            'Tong so ban ghi
            If p.Count > 0 Then
...
#Region "Event for control"
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Using data As New ThanhTraLaoDongEntities
            intId = grdShow.DataKeys(hidID.Value)("FuncId").ToString

            Dim q = (From p In data.Functions Where p.FunctionId = intId Select p).FirstOrDefault
            Try
                data.Functions.DeleteObject(q)
                data.SaveChanges()
                Insert_App_Log("Delete Function:" & q.HrefName & "", Function_Name.FunctionSys, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
            ...
            Try
                For Each item As GridViewRow In grdShow.Rows
                    ...
                    If chkItem.Checked Then
                        intTotal += 1
                        intId = grdShow.DataKeys(item.RowIndex)("FuncId").ToString
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            ...
            hplEdit.NavigateUrl = "../../Page/Function/Edit.aspx?FuncId=" & grdShow.DataKeys(e.Row.RowIndex)("FunctionId").ToString
            Dim hplHrefNameSub As HyperLink = CType(e.Row.FindControl("hplHrefNameSub"), HyperLink)
            hplHrefNameSub.NavigateUrl = "../../Page/Function/Detail.aspx?FuncId=" & grdShow.DataKeys(e.Row.RowIndex)("FunctionId").ToString
            hplHrefNameSub.Text = grdShow.DataKeys(e.Row.RowIndex)("FunctionName").ToString

            Dim chkItem As CheckBox = CType(e.Row.FindControl("chkItem"), CheckBox)
            'Permission
            hplEdit.Enabled = HasPermission(Function_Name.FunctionSys, Session("RoleID"), 0, Audit_Type.Edit)
