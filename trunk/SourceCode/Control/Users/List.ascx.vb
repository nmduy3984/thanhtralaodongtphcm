...
#Region "Sub and Function"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1,
                           ...
                           Optional ByVal iTinhId As Integer = 0)
        Using data As New ThanhTraLaoDongEntities
            Dim arrSearch() As String = {iPage.ToString, strSearch, iTinhId.ToString}
            ViewState("search") = arrSearch
            'So ban ghi muon the hien tren trang
            Dim intPag_Size As Int32 = drpPage_Size.SelectedValue
            Dim qr = From q In data.Users
                     Where (iTinhId = 0 Or q.TinhTP = iTinhId)
                     Order By q.IsUser Descending, q.UserName Ascending
                     Select q
    ...
    Protected Sub lnkTitle_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        BindToGrid(CInt(lnkTile.ToolTip.ToString), arrSearch(1).ToString, arrSearch(2).ToString)
    ...
    Protected Sub lnkFirst_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkFirst.Click
        ...
        Dim arrSearch() As String
        arrSearch = ViewState("search")
        BindToGrid(CInt(hidCur_Page.Value.ToString), arrSearch(1).ToString, arrSearch(2).ToString)
    End Sub
...
#Region "Event for control"
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim arrSearch() As String
            arrSearch = ViewState("search")
            BindToGrid(CInt(arrSearch(0).ToString), arrSearch(1).ToString, arrSearch(2).ToString)
        End Using
    End Sub
                    ...
                    If chkItem.Checked Then
                        ...
                        Try
                            ...
                            If gy.Count = 0 Then
                                'Delete user in UserTinh table
                                Dim ut = (From a In data.UsersTinhs Where a.UserId = intId).ToList()
                                For Each temp As UsersTinh In ut
                                    data.UsersTinhs.DeleteObject(temp)
                                Next

                                'Delete user in UserRole table
                                Dim ur = (From a In data.UserRoles Where a.UserId = intId).ToList
                                For Each temp As UserRole In ur
                                    data.UserRoles.DeleteObject(temp)
                                Next
                                data.SaveChanges()
...
#Region "Search"
    ...
    Protected Sub drpPage_Size_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles drpPage_Size.SelectedIndexChanged
        Dim arrSearch() As String
        arrSearch = ViewState("search")
        BindToGrid(1, arrSearch(1).ToString, arrSearch(2).ToString)
    End Sub
