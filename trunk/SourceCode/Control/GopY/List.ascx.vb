...
Partial Class Control_GopY_List
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1, Optional ByVal strSearch As String = "")
        Using data As New ThanhTraLaoDongEntities
            ...
            If TongSoUserDanhGia > 0 Then
                lblThongKe.Text = "<ul><b>Kết quả đánh giá</b>"
                lblThongKe.Text += "<li>1. Rất hay: <span style='color:blue;'><b>" & PerLevel1 & "</b></span> %<li>"
                lblThongKe.Text += "<li>2. Trung bình: <span style='color:blue;'><b>" & PerLevel2 & "</b></span> %<li>"
                lblThongKe.Text += "<li>3. Không cần: <span style='color:blue;'><b>" & PerLevel3 & "</b></span> %<li>"
                lblThongKe.Text += "<li>4. Số người đánh giá: <span style='color:blue;'><b>" & String.Format(info, "{0:n0}", TongSoUserDanhGia) & "</b></span> người<li>"
                lblThongKe.Text += "</ul>"
            End If
        End Using
    End Sub
...
#Region "Event for control"
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Using data As New ThanhTraLaoDongEntities
            intId = grdShow.DataKeys(hidID.Value)("GopYId").ToString

            Dim q = (From p In data.Gopies Where p.GopYId = intId Select p).FirstOrDefault
            Try
                data.Gopies.DeleteObject(q)
                data.SaveChanges()
                Insert_App_Log("Delete GopY:" & q.GopYId & "", Function_Name.GopY, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
            ...
            Try
                For Each item As GridViewRow In grdShow.Rows
                    ...
                    If chkItem.Checked Then
                        intTotal += 1
                        intId = grdShow.DataKeys(item.RowIndex)("GopYId").ToString
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            ...
            'Permission
             If HasPermission(Function_Name.GopY, Session("RoleID"), 0, Audit_Type.Delete) = True Then
