...
Partial Class Control_TraCuu_DSDNChuaKhamSucKhoeDinhKy
...
#Region "PRIVATE EVENT FOR CONTROL"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1 _
        ...
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim p = data.uspTraCuuDSDNChuaKhamSucKhoeDinhKy(strTinh, strYearFrom, iPage, intPag_Size).ToList()
...
#Region "Export Excel"
    Private Sub ExportExcel()
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim intDiaPhuong As Integer = ddlDiaPhuong.SelectedValue
            Dim intYearFrom As Integer = ddlYearFrom.SelectedValue
            Dim p = data.uspTraCuuDSDNChuaKhamSucKhoeDinhKy(intDiaPhuong, intYearFrom, 1, 999999).ToList()
            Try

                identity = Now.ToString("ddMMyyyyHHmmss")
                pathFile = FolderPath & "\DSDNChuaKhamSucKhoeDinhKy.xlsx"
                fileNameTemp = "DSDNChuaKhamSucKhoeDinhKy" & "_" & identity & ".xlsx"
                ...
                If File.Exists(pathFile) Then
                    ...
                    Dim arrRange(p.Count, 7) As Object
                    For pos As Integer = 0 To p.Count - 1
                        ...
                        arrRange(pos, col) = ""
                        col += 1
                        arrRange(pos, col) = IIf(IsNothing(p(pos).SoNguoiPhaiKham), "0", p(pos).SoNguoiPhaiKham)
                    Next
                    'B2: Gán mảng vào sheet

                    If arrRange.Length > 0 Then
                        _sheet_Detail.Range("A1", "A1").Value = "Danh sách doanh nghiệp chưa khám sức khỏe định kỳ năm " + ddlYearFrom.SelectedValue.ToString
                        Dim arrSheet As Excel.Range
                        arrSheet = _sheet_Detail.Range("A4", "G" & +(p.Count + 3).ToString())
                        With arrSheet
                            .Value2 = arrRange
