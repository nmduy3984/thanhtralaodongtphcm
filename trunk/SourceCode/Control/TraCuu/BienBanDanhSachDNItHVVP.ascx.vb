...
Partial Class Control_TraCuu_BienBanDanhSachDNItHVVP
    ...
    Protected Sub getYear()
        ...
        ddlYearTo.Items.Add(New ListItem("---Tất cả---", "2900"))
        ddlYearTo.DataSource = Enumerable.Range(2005, 11)
        ddlYearTo.DataBind()
    ...
    Protected Sub LoadType()
        Using Data As New ThanhTraLaoDongEntities
            ...
            Dim lstChuyenMuc = (From q In Data.vLoaiHanhVis Select New With {.Text = q.Name, .Value = q.Id})
            With ddlChuyenMuc
                .Items.Clear()
                .AppendDataBoundItems() = True
                .DataTextField = "Text"
                .DataValueField = "Value"
                .DataSource = lstChuyenMuc
                .DataBind()
...
#Region "PRIVATE EVENT FOR CONTROL"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1 _
                            ...
                            , Optional ByVal strChuyenMuc As Integer = 0 _
                            , Optional ByVal strViPham As Boolean = False)
        Using data As New ThanhTraLaoDongEntities
            Dim arrSearch() As String = {iPage.ToString, strTinh.ToString, strYearFrom.ToString, strYearTo.ToString, strChuyenMuc.ToString, strViPham.ToString}

            ViewState("search") = arrSearch
            'So ban ghi muon the hien tren trang
            Dim intPag_Size As Int32 = drpPage_Size.SelectedValue
            Dim p = data.uspBienBanDanhSachDNCoItHVVP(strTinh, strYearFrom, strYearTo, strChuyenMuc, strViPham, iPage, intPag_Size).ToList()
            'Tong so ban ghi
            If p.Count > 1 Then
    ...
    Private Sub Button_Click(ByVal sender As Object, ByVal em As System.EventArgs) Handles btnExport.Click
        ...
        Dim intChuyenMuc As Integer = ddlChuyenMuc.SelectedValue
        Dim isViPham As Boolean = rptViPham.SelectedValue = 1
        BindToGrid(1, intDiaPhuong, intYearFrom, intYearTo, intChuyenMuc, isViPham)
    End Sub
