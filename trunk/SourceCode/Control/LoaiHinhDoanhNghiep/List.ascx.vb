...
Partial Class Control_LoaiHinhDoanhNghiep_List
...
#Region "Sub and Function"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1, Optional ByVal strSearch As String = "")
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim p = (From q In data.LoaiHinhDoanhNghieps Where q.TenLoaiHinhDN.Contains(strSearch) Order By q.LoaiHinhDNId Ascending Select q).Skip((iPage - 1) * intPag_Size).Take(intPag_Size).ToList
            Dim strKey_Name() As String = {"LoaiHinhDNId", "TenLoaiHinhDN"}
            'Tong so ban ghi
            If strSearch = "" Then
                hidCount.Value = data.LoaiHinhDoanhNghieps.Count
            Else
...
#Region "Event for control"
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Using data As New ThanhTraLaoDongEntities
            intId = grdShow.DataKeys(hidID.Value)("LoaiHinhDNId").ToString

            Dim q = (From p In data.LoaiHinhDoanhNghieps Where p.LoaiHinhDNId = intId Select p).FirstOrDefault
            Try
                Dim dn = (From a In data.DoanhNghieps Where a.LoaiHinhDNId = intId).ToList
                If dn.Count = 0 Then
                    data.LoaiHinhDoanhNghieps.DeleteObject(q)
                    data.SaveChanges()
                    Insert_App_Log("Delete Loai hinh doanh nghiep:" & q.TenLoaiHinhDN & "", Function_Name.LoaiHinhDN, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
                    Excute_Javascript("Alertbox('Xóa dữ liệu thành công.');", Me.Page, True)
                Else
                    Excute_Javascript("Alertbox('Xóa thất bại. Loại hình doanh nghiệp này hiện tại có doanh nghiệp tham chiếu đến.');", Me.Page, True)
                End If
            ...
            Try
                For Each item As GridViewRow In grdShow.Rows
                    ...
                    If chkItem.Checked Then
                        intTotal += 1
                        intId = grdShow.DataKeys(item.RowIndex)("LoaiHinhDNId").ToString
                        Dim q = (From p In data.LoaiHinhDoanhNghieps Where p.LoaiHinhDNId = intId Select p).FirstOrDefault
                        Dim dn = (From a In data.DoanhNghieps Where a.LoaiHinhDNId = intId).ToList
                        If dn.Count = 0 Then
                            data.LoaiHinhDoanhNghieps.DeleteObject(q)
                            ...
                            intCount += 1
                        Else
                            strLHDN += q.TenLoaiHinhDN & ";"
                        End If
                    End If
                Next
                If intCount > 0 Then
                    Excute_Javascript("Alertbox('Xóa dữ liệu thành công." & intCount.ToString & " /" & intTotal.ToString & " record. " & If(Not strLHDN.Equals(""), " Các Loại hình doanh nghiệp (" & strLHDN & ") hiện tại có doanh nghiệp tham chiếu đến", "") & "');", Me.Page, True)
                Else
                    Excute_Javascript("Alertbox('Xóa thất bại." & If(Not strLHDN.Equals(""), " Các Loại hình doanh nghiệp (" & strLHDN & ") hiện tại có doanh nghiệp tham chiếu đến", "") & "');", Me.Page, True)
                End If
            Catch ex As Exception
                log4net.Config.XmlConfigurator.Configure()
                log.Error("Error error " & AddTabSpace(1) & Session("Username") & AddTabSpace(1) & "IP:" & GetIPAddress(), ex)
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            ...
            hplEdit.NavigateUrl = "../../Page/LoaiHinhDoanhNghiep/Edit.aspx?LoaiHinhDNId=" & grdShow.DataKeys(e.Row.RowIndex)("LoaiHinhDNId").ToString
            Dim hplTitle As HyperLink = CType(e.Row.FindControl("hplTitle"), HyperLink)
            hplTitle.NavigateUrl = "../../Page/LoaiHinhDoanhNghiep/Detail.aspx?LoaiHinhDNId=" & grdShow.DataKeys(e.Row.RowIndex)("LoaiHinhDNId").ToString
            hplTitle.Text = grdShow.DataKeys(e.Row.RowIndex)("TenLoaiHinhDN").ToString

            Dim chkItem As CheckBox = CType(e.Row.FindControl("chkItem"), CheckBox)
            'Permission
            hplEdit.Enabled = HasPermission(Function_Name.LoaiHinhDN, Session("RoleID"), 0, Audit_Type.Edit)
            If HasPermission(Function_Name.LoaiHinhDN, Session("RoleID"), 0, Audit_Type.Delete) = True Then
