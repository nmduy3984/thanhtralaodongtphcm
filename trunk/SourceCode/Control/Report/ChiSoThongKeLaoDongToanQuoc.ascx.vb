    ...
    Private Sub ExportExcel()
        Using data As New ThanhTraLaoDongEntities
            ...
            Try
                ...
                pathFile = FolderPath & "\ChiSoThongKe.xlsx"
                fileNameTemp = "ChiSoThongKe" & "_" & identity & ".xlsx"
                ...
                If File.Exists(pathFile) Then
                    ...
                    For pos As Integer = 0 To p.Count - 1
                        ...
                        col += 1
                        If IsNothing(p(pos).TieuChi) Then
                            arrRange(pos, col) = ""
                        Else
                            arrRange(pos, col) = p(pos).TieuChi
                        End If

                        col += 1
                        If IsNothing(p(pos).BienBan) Then
                            arrRange(pos, col) = ""
                        Else
                            arrRange(pos, col) = String.Format("{0:n2}", CType(p(pos).BienBan, Decimal))
                        End If

                        col += 1
                        If IsNothing(p(pos).Phieu) Then
                            arrRange(pos, col) = ""
                        Else
                            arrRange(pos, col) = String.Format("{0:n2}", p(pos).Phieu)
                        End If

                        col += 1
                        If IsNothing(p(pos).Chung) Then
                            arrRange(pos, col) = ""
                        Else
                            arrRange(pos, col) = String.Format("{0:n2}", p(pos).Chung)
                        End If
                     Next
                    'B2: Gán mảng vào sheet

                    If arrRange.Length > 0 Then
                        _sheet_Detail.Range("A1", "A1").Value = "Chỉ số thống kê lao động thành phố Hồ Chí Minh năm " + ddlNam.SelectedValue.ToString
