...
<script type="text/javascript">
    function ajaxJquery() {
        $(function () {
            var ddlNam = $("#<%=ddlNam.ClientID %>").find("option:selected").val();
            if (ddlNam * 1 > 0) {
                $("#<%=lblReportName.ClientID %>").text('Chỉ số thống kê lao động thành phố Hồ Chí Minh năm ' + ddlNam);
            }
            else {
                $("#<%=lblReportName.ClientID %>").text('Chỉ số thống kê lao động thành phố Hồ Chí Minh theo năm');
            }

            $("#<%=ddlNam.ClientID %>").change(function (event) {
                var valueType = $("#SearchType option:selected").val();
                if (valueType * 1 > 0) {
                    $("#<%=lblReportName.ClientID %>").text('Chỉ số thống kê lao động thành phố Hồ Chí Minh năm ' + valueType);
                }
                else {
                    $("#<%=lblReportName.ClientID %>").text('Chỉ số thống kê lao động thành phố Hồ Chí Minh theo năm');
                }
...
<asp:UpdatePanel ID="uplDailyDebt" runat="server">
    <ContentTemplate>
        <div class="BoxField">
            <div class="HeadTitle">
                <h3>
                    <asp:Label ID="lblReportName" runat="server" Text="Chỉ số thống kê lao động thành phố Hồ Chí Minh theo năm" />
                </h3>
            </div>
