...
Partial Class Control_DanhMucTrichDan_List
...
#Region "Sub and Function"
    ...
    Private Sub BindToGrid(Optional ByVal iPage As Integer = 1, Optional ByVal strSearch As String = "")
        Using data As New ThanhTraLaoDongEntities
            ...
            Dim p = data.uspDMTrichDanSelectAll(strSearch, iPage, intPag_Size).ToList()
            Dim strKey_Name() As String = {"TrichDanId", "NDTrichDan", "CauHoiVietTat"}
            'Tong so ban ghi
            If p.Count > 0 Then
...
#Region "Event for control"
    ...
    Protected Sub lnkbtnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Using data As New ThanhTraLaoDongEntities
            intId = grdShow.DataKeys(hidID.Value)("TrichDanId").ToString

            Dim q = (From p In data.DanhMucTrichDans Where p.TrichDanId = intId Select p).FirstOrDefault
            Try
                data.DanhMucTrichDans.DeleteObject(q)
                data.SaveChanges()
                Insert_App_Log("Delete Danhmuctrichdan:" & q.TrichDanId & "", Function_Name.DanhMucTrichDan, Audit_Type.Delete, Request.ServerVariables("REMOTE_ADDR"), Session("UserName"))
            ...
            Try
                For Each item As GridViewRow In grdShow.Rows
                    ...
                    If chkItem.Checked Then
                        intTotal += 1
                        intId = grdShow.DataKeys(item.RowIndex)("TrichDanId").ToString
    ...
    Protected Sub grdShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdShow.RowDataBound
        If e.Row.RowIndex >= 0 Then
            ...
            hplEdit.NavigateUrl = "../../Page/DanhMucTrichDan/Edit.aspx?TrichDanId=" & grdShow.DataKeys(e.Row.RowIndex)("TrichDanId").ToString & "&Mod="
            Dim hplNDTrichDan As HyperLink = CType(e.Row.FindControl("hplNDTrichDan"), HyperLink)
            hplNDTrichDan.NavigateUrl = "../../Page/DanhMucTrichDan/Detail.aspx?TrichDanId=" & grdShow.DataKeys(e.Row.RowIndex)("TrichDanId").ToString
            hplNDTrichDan.Text = grdShow.DataKeys(e.Row.RowIndex)("NDTrichDan").ToString
            'View mục vi phạm
            ...
            'Permission
            hplEdit.Enabled = HasPermission(Function_Name.DanhMucTrichDan, Session("RoleID"), 0, Audit_Type.Edit)
            If HasPermission(Function_Name.DanhMucTrichDan, Session("RoleID"), 0, Audit_Type.Delete) = True Then
